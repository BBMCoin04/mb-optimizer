...
第一轮只做延迟和丢包测试
      ↓
取前 30 个建立下载候选池，最多找出 10 个非零速度结果
      ↓
按速度、丢包、延迟取前 3 个
      ↓
前 3 个额外复测 2 轮
...
- 默认使用 Cloudflare 官方 Speed Test 下载端点。
- 备用使用 Cloudflare 镜像站稳定大文件。
- 普通域名预检只作诊断，最终由 CFST 通过候选 IP 实测并自动切换。
...
- 7 天缓存、过期缓存和内置三级回退
- 限量 800 地址延迟广筛
- 30 个下载候选池、10 个非零速度目标、3 个最终复测地址
- CFST 回车进度实时解析
- 主进度条、当前数量、可用数量和已用时间
- 可拖动及放大的日志区域
- 柔和浅色背景
